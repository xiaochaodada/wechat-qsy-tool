...
Page({
    ...
    toSplic: function () {
        ...
        o - t <= 2 && !a ? wx.showToast({
            title: "保留的视频不能小于3秒哦",
            icon: "none"
        }) : o != i || 0 != t ? a ? wx.showModal({
            content: "您确定要裁剪掉视频的" + t + "到" + o + "秒",
            success: function (t) {
                t.confirm && e.login();
            }
        }) : wx.showModal({
            content: "您确定只要保留视频的" + t + "到" + o + "秒",
            success: function (t) {
                t.confirm && e.login();
            }
        }) : wx.showToast({
    ...
    submit: function (code) {
        ...
        if (hash_data[0].length < 5) {
            ...
            var kaishi = that.data.low,
                jieshu = that.data.heigh;
            var payload = '{"v":"' + hash_data[0] + '","s":"' + kaishi + '","e":"' + jieshu + '"}';
            console.log(payload)
            ...
            wx.request({
                ...
                success: function (res) {
                    ...
                    if (res.statusCode == 200 || res.statusCode == 201) {
                        ...
                        cache.record(res.data.result.id, 8, (json_upload_data.duration).toFixed(2), (json_upload_data.size / 1024 / 1024).toFixed(2), res.data.result.created_at, res.data.result.expired_at)
